...
def reachable_station_indivisual(i):
    ...
    for j in range(30):
        ...
        if dist > v_soc:
            v_map[vid].append([s_id.sid, s_id.coc, 1000])# 1000 is considered infinity here (Station not reachable)
        else:
            v_map[vid].append([s_id.sid, s_id.coc, dist])
            s_map[s_id.sid].append([vid, dist])

        wql.append(len(s_id.queue))
        cost.append(s_id.coc)
        Dist.append(dist)

    return wql, cost, Dist
...
def wait_count(index, message, requests):
    ...
    for i in range(30):
        ...
        if len(s_id.queue):
            wait.append(len(s_id.queue))  # -1, since first vehicle is not waiting
        else:
            wait.append(0)
...
def fuzzy_logic_wq_dist_cost(wql, d, price):
    wait = ctrl.Antecedent(np.arange(0, 10, 1), 'waiting queue length')
    dist = ctrl.Antecedent(np.arange(0, 100, 1), 'distance')
    cost = ctrl.Antecedent(np.arange(6, 10, 0.1), 'cost')
    result = ctrl.Consequent(np.arange(0, 101, 1), 'result')

    wait['low'] = fuzz.trimf(wait.universe, [0, 0, 10])
    wait['medium'] = fuzz.trimf(wait.universe, [1, 10, 20])
    wait['high'] = fuzz.trimf(wait.universe, [10, 20, 20])

    dist['low'] = fuzz.trimf(dist.universe, [0, 0, 50])
    dist['medium'] = fuzz.trimf(dist.universe, [25, 50, 75])
    dist['high'] = fuzz.trimf(dist.universe, [51, 100, 100])

    cost['low'] = fuzz.trimf(cost.universe, [6, 6, 7.5])
    cost['medium'] = fuzz.trimf(cost.universe,[7, 8, 9.5])
    ...
    rule1 = ctrl.Rule(wait['low'] & cost['low'] & dist['low'], result['high'])
    ...
    rule10 = ctrl.Rule(wait['medium'] & cost['low'] & dist['low'], result['high'])
    ...
    rule13 = ctrl.Rule(wait['medium'] & cost['medium'] & dist['low'], result['medium'])
    ...
    rule18 = ctrl.Rule(wait['medium'] & cost['high'] & dist['high'], result['low'])

    rule19 = ctrl.Rule(wait['high'] & cost['low'] & dist['low'], result['low'])
...
def three_d_plot(p1, p2, m1, m2, index):
    ...
    sid = np.arange(1, 31, 1)
    x = p1
    y = p2

    # 3D Scatter Plot
    ax.scatter(x, y, sid, c='r', s=50)
    ax.set_title('3D Scatter Plot for vehicle '+str(index))

    # Wireframe plot
    # ax.plot_trisurf(x, y, sid, cmap='Blues', linewidth=0, antialiased=False)
    # supported values are 'Accent', 'Accent_r', 'Blues', 'Blues_r', 'BrBG', 'BrBG_r', 'BuGn', 'BuGn_r', 'BuPu',
    # 'BuPu_r', 'CMRmap', 'CMRmap_r', 'Dark2', 'Dark2_r'
    # ax.set_title('Surface plot for vehicle '+str(index))

    # Set axes label
    ax.set_xlabel(m1, labelpad=20)
    ax.set_ylabel(m2, labelpad=20)
    ax.set_zlabel('sid', labelpad=20)
...
if __name__ == "__main__":
    ...
    for x in range(4):
        if x == 0:
            ...
            reachable_station(request[:40])

            # waiting queue length count wl_50_wait=wait queue length when request is 50 and wait is considered
            wl_50_wait = wait_count(3, "wait", 40)   # 3 because in reachable station at index 3 wait time is stored

        elif x == 1:
            wql, cost, Dist = reachable_station_indivisual(40)
            three_d_plot(wql, cost, 'wql', 'price', 40)
            three_d_plot(cost, Dist, 'price', 'distance', 40)
            three_d_plot(wql, Dist, 'wql', 'distance', 40)

        elif x == 2:
            wql, cost, Dist = reachable_station_indivisual(41)
            three_d_plot(wql, cost, 'wql', 'price', 41)
            three_d_plot(cost, Dist, 'price', 'distance', 41)
            three_d_plot(wql, Dist, 'wql', 'distance', 41)

        elif x == 3:
            wql, cost, Dist = reachable_station_indivisual(42)
            three_d_plot(wql, cost, 'wql', 'price', 42)
            three_d_plot(cost, Dist, 'price', 'distance', 42)
            three_d_plot(wql, Dist, 'wql', 'distance', 42)
